<template>
  <CrmDrawer v-model:show="visible" resizable no-padding :width="800" :footer="false">
    <template v-if="!props.readonly" #titleRight>
      <n-button
        v-permission="['OUTSOURCING:UPDATE']"
        type="primary"
        ghost
      ...
      <n-button
        v-permission="['OUTSOURCING:DELETE']"
        type="error"
        ghost
    ...
    <div class="h-full bg-[var(--text-n9)] px-[16px] pt-[16px]">
      <CrmCard hide-footer>
        <div class="flex-1">
          <CrmFormDescription
            :form-key="FormDesignKeyEnum.OUTSOURCING"
...
<script lang="ts" setup>
  ...
  function handleInit(_type?: any, _name?: string, detail?: Record<string, any>) {
