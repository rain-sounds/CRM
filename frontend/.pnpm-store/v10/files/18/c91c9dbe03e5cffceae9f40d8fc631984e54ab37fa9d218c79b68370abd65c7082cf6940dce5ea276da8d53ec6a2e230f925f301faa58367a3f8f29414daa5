 ...
 */
const boundary = (line, view, magnet, options) => {
    let node;
    let intersection;
    ...
    if (typeof selector === 'string') {
        ...
        node = common_1.ObjectExt.getByPath(magnet, selector);
    }
    else {
        node = (0, util_1.findShapeNode)(magnet);
    }
    if (!common_1.Dom.isSVGGraphicsElement(node)) {
        if (node === magnet || !common_1.Dom.isSVGGraphicsElement(magnet)) {
            return anchor;
        }
        node = magnet;
    }
    ...
    const localLine = common_1.Util.transformLine(line, localMatrix);
    ...
    const cp = intersection
        ? common_1.Util.transformPoint(intersection, targetMatrix)
        : anchor;
