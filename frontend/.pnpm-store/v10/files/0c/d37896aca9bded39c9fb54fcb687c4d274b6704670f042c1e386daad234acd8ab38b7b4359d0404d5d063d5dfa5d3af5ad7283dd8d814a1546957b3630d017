...
import { Dom, disposable, NumberExt } from '../../common';
...
export class TransformImpl extends View {
    ...
    updateResizerDirections() {
        ...
        if (shift !== this.prevShift) {
            ...
            const directions = DIRECTIONS.slice(shift).concat(DIRECTIONS.slice(0, shift));
            ...
            resizes.forEach((resize, index) => {
                Dom.removeClass(resize, DIRECTIONS.map((dir) => className(dir)).join(' '));
