...
let jumppedLines = [];
let skippedPoints = [];
export function setupUpdating(view) {
...
export function createLines(sourcePoint, targetPoint, route = []) {
...
export function findLineIntersections(line, crossCheckLines) {
    const intersections = [];
...
export function getDistence(p1, p2) {
    return new Line(p1, p2).squaredLength();
}
/**
 * Split input line into multiple based on intersection points.
 */
export function createJumps(line, intersections, jumpSize) {
...
export function buildPath(lines, jumpSize, jumpType, radius) {
    const path = new Path();
    let segment;
...
export function buildRoundedSegment(offset, path, curr, prev, next) {
