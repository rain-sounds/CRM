 ...
 * ```
 */
export declare function createTodo(params?: APIParams<{
	/**
	 * 待办内容（标题）。建议不超过一行可见长度，过长会被截断显示
	 */
	content?: string;
	/**
	 * 参与人 userid 列表。需为当前应用可见范围内的企业成员
	 */
	attendees?: string[];
	/**
	 * 截止时间，unix 时间戳，单位：秒
 ...
 * ```
 */
export declare function viewTodo(params: APIParams<{
	/**
	 * 待办 id，由 createTodo 创建成功后回调返回
	 */
	todoId: string;
}>): Promise<CommonResult>;
/**
...
export interface ImageMessage {
	...
	image: {
		 ...
		 * 图片的素材 ID
		 *
		 * 可以通过[素材管理](#10115)接口获得，暂不支持公众平台的 mediaid
		 */
		mediaid?: string;
		/**
...
export declare enum OpenAppManagePageType {
	 ...
	 */
	datazone_chat_permission = "datazone_chat_permission",
	/**
	 * 数据专区-文档权限授权页
	 *
	 * 需要满足：
	 ...
	 */
	datazone_doc_permission = "datazone_doc_permission",
	/**
	 * 数据专区-知识集权限授权页
	 *
	 * 需要满足：
	 ...
	 * - 应用类型为第三方应用（注：不支持上下游共享应用）
	 */
	datazone_knowledge_permission = "datazone_knowledge_permission",
	/**
	 * 数据与智能专区权限授权页
	 *
	 ...
	 * - 应用类型为第三方应用/代开发应用（注：不支持上下游共享应用）
	 */
	customer_acquisition_permission = "customer_acquisition_permission"
}
/**
 * 打开应用管理页面。
 *
...
export declare enum DatazoneDocType {
	/**
	 * 其他
	 */
	other = 0,
 ...
 * ```
 */
export declare function selectDatazoneDoc(params?: APIParams<IEmptyObject, {
	/**
	 * 返回的文档列表
	 */
	docList: Array<{
 ...
 * ```
 */
export declare function selectDatazoneFile(params?: APIParams<IEmptyObject, {
	/**
	 * 用户选中的微盘文件列表
	 */
	fileList: Array<{
		/**
		 * 选中文件的 fileId。
		 * 不同调用方（企业自建 vs 第三方服务商）拿到的 fileId 不通用
		 */
		fileId: string;
		/**
		 * 选中文件的访问 url
 ...
 * ```
 */
export declare function selectExternalChat(params?: APIParams<IEmptyObject, {
	/**
	 * 单次最多可选择 5 个
	 */
	chatIdList?: string[];
...
export type PANEL_JSAPI = {
	selectEnterpriseContact: typeof selectEnterpriseContact;
	shareAppMessage: typeof shareAppMessage;
};
 ...
 */
export declare function createJSAPIPanel<TName extends keyof PANEL_JSAPI>(name: TName, options: JSAPIPanelOptions<PANEL_JSAPI[TName]>): {
	/**
