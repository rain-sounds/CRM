...
import { Basecoat, disposable, FunctionExt, ObjectExt, } from '../../common';
import { Model } from '../../model';
import { getOptions, isAddEvent, isChangeEvent, isRemoveEvent, sortBatchCommands, } from './util';
...
export class History extends Basecoat {
    ...
    executeCommand(cmd, revert, options) {
        ...
        else if ((isAddEvent(event) && !revert) ||
    ...
    addCommand(event, args) {
        ...
        if ((isAddEvent(event) && this.options.ignoreAdd) ||
    ...
    filterBatchCommand(batchCommands) {
        ...
        while (cmds.length > 0) {
            ...
            if (evt != null && (id != null || cmd.modelChange)) {
                if (isAddEvent(evt)) {
                    const index = cmds.findIndex((c) => isRemoveEvent(c.event) && c.data.id === id);
                    if (index >= 0) {
                    ...
                    const index = cmds.findIndex((c) => isAddEvent(c.event) && c.data.id === id);
                    if (index >= 0) {
                        cmds.splice(index, 1);
                        continue;
                    }
