...
import { isValidTarget, removeHandlerId, addEventListener, ensureHandlerId, splitType, normalizeType, removeEventListener, getHandlerId, isWindow, setHandlerId, getHandlerQueue, stopPropagationCallback, } from './util';
...
import './special';
let triggered;
export function on(elem, types, handler, data, selector) {
    ...
    splitType(types).forEach((item) => {
        ...
        if (!bag) {
            ...
            if (!hook.setup ||
                ...
                addEventListener(elem, type, mainHandler);
            }
        }
        if (hook.add) {
...
export function off(elem, types, handler, selector, mappedTypes) {
    ...
    splitType(types).forEach((item) => {
        ...
        if (originHandlerCount && bag.handlers.length === 0) {
            if (!hook.teardown ||
                ...
                removeEventListener(elem, type, store.handler);
            }
            delete events[type];
        }
    });
...
export function dispatch(elem, evt, ...args) {
...
export function trigger(event, eventArgs, elem, onlyHandlers) {
    let eventObj = event;
    ...
    for (let i = 0, l = eventPath.length; i < l && !eventObj.isPropagationStopped(); i += 1) {
        ...
        // Custom handler
        const store = getStore(currElement);
        if (store) {
