...
class Store extends common_1.Basecoat {
    constructor(data = {}) {
        super();
        this.pending = false;
        this.changing = false;
        this.data = {};
        this.mutate(common_1.ObjectExt.cloneDeep(data));
        this.changed = {};
    }
    mutate(data, options = {}) {
        ...
        this.changing = true;
        if (!changing) {
            this.previous = common_1.ObjectExt.cloneDeep(this.data);
            this.changed = {};
        }
        ...
        Object.keys(data).forEach((k) => {
            ...
            if (!common_1.ObjectExt.isEqual(current[key], newValue)) {
                changes.push(key);
            }
            if (!common_1.ObjectExt.isEqual(previous[key], newValue)) {
    ...
    getByPath(path) {
        return common_1.ObjectExt.getByPath(this.data, path, '/');
    }
    setByPath(path, value, options = {}) {
        ...
        if (pathArrayLength === 1) {
            ...
            common_1.ObjectExt.setByPath(update, pathArray, value, delim);
            const data = common_1.ObjectExt.cloneDeep(this.data);
            // If rewrite mode enabled, we replace value referenced by path with the
            // new one (we don't merge).
            if (options.rewrite) {
                common_1.ObjectExt.unsetByPath(data, path, delim);
            }
            const merged = common_1.ObjectExt.merge(data, update);
    ...
    removeByPath(path, options) {
        ...
        if (keys.length === 1) {
            ...
            const prop = common_1.ObjectExt.cloneDeep(this.get(key));
            if (prop) {
                common_1.ObjectExt.unsetByPath(prop, paths);
            }
    ...
    getChanges(diff) {
        if (diff == null) {
            return this.hasChanged() ? common_1.ObjectExt.cloneDeep(this.changed) : null;
        }
        ...
        for (const key in diff) {
            const val = diff[key];
            if (!common_1.ObjectExt.isEqual(old[key], val)) {
                changed[key] = val;
                hasChanged = true;
            }
        }
        return hasChanged ? common_1.ObjectExt.cloneDeep(changed) : null;
    }
    /**
     * Returns a copy of the store's `data` object.
     */
    toJSON() {
        return common_1.ObjectExt.cloneDeep(this.data);
    }
    clone() {
